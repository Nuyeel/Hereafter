...
router.route('/ajax/axios/cwb')
    .get((req, res) => {
        // 將中文字轉成 unicode 做跳脫 (escape)
        // Server 盡量不要有中文字
        const city = encodeURI('臺北');

        axios.get(`https://opendata.cwb.gov.tw/api/v1/rest/datastore/O-A0003-001?Authorization=CWB-E2349193-68E5-4124-B14D-C0226B44B958&locationName=${city}`).then((response) => {
            // 這裡只做到取得資料 沒有存入資料庫
            res.send(response.data);
        }
    );
